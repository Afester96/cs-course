...
namespace HomeWork4
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Значения 111 для обработки
            bottles full = bottles.oneIter | bottles.fiveIter | bottles.twentyIiter;

            if (ceilingEnteredAmount > 0)
            {
                if (full != 0)
                {
                    if (amountOneLiterBottle > 0 & amountFiveLiterBottle > 0 & amountTwentyLiterBottle > 0) //все бутылки
                    {
                    ...
                    {
                        Console.WriteLine("Приносим наши извинения, у нас закончились контейнеры на складе, закажите позже!");
                    }
                }
                else
                {
